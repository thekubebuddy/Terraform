...
resource "null_resource" "null_id" {
  provisioner "local-exec" {
    ...
    command = "docker cp ./bootstrap.sh ${docker_container.container_1.name}:/tmp/ && docker exec ${docker_container.container_1.name} bash -c 'ls /tmp/;chmod +x /tmp/bootstrap.sh;/tmp/bootstrap.sh' && docker cp ./index.html ${docker_container.container_1.name}:/var/www/html/"
